...
pub fn upload_config () -> Config {
  let mut file = match File::open("/etc/fin-config/config.json") {
    Err(error)=>{
      match error.kind() {
        std::io::ErrorKind::NotFound => return Config {background:false,underline:true,color:ConfigColor::Green},
        _=>{
          println!("{}",error);
          process::exit(1);
        }
  ...
  file.read_to_string(&mut row_config).unwrap_or_else(|error| {
    match error.kind() {
